...
5. **Exploit** — Write a pwntools exploit script and test it with `run_exploit`. On **menu-driven** or **multi-prompt** binaries, the first `run_exploit` script should be a **reachability / sync harness only**: prove you can read the banner, match the menu prompt, select the target option, satisfy intermediate prompts, and return or exit cleanly. Instrument this harness with explicit checkpoint prints and `repr()` of received bytes so prompt mismatches are visible in tool output. Only after that succeeds should you send cyclic patterns, leaks, or real payloads. Prefer `sendlineafter` / `sendafter` / `recvuntil` helpers over one giant stdin blob.
